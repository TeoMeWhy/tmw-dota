...
class CollectorMatch:
    ...
    def exec_collect(self):
        ...
        if resp.status_code == 200:
            self.save_matches(resp.json())
            return True
        return False


    def exec_collect_until(self, date='2026-04-01', from_history=True):

        last_id = None
        if from_history:
            last_id = db_models.get_oldest_match_id(self.engine)
        ...
        while date <= dt_match:
            ...
            if resp.status_code != 200:
                print(f"Erro na chamada da API: {resp.text}")
                print("Aguardando alguns segundos para retomar...")
                time.sleep(60)
                continue
